...
@interface LEHtmlMarkupStripper () <NSXMLParserDelegate>

@property (assign, nonatomic) BOOL parseError;
@property (strong, nonatomic) NSMutableArray* parseBuffer;

@end


@implementation LEHtmlMarkupStripper

- (void)parser          :(NSXMLParser*)parser
         foundCharacters:(NSString*)string {
    [self.parseBuffer addObject:string];
}
...
- (NSString*)parse:(NSString*)text {
    self.parseBuffer = [[NSMutableArray alloc] init];
    ...
    if (!self.parseError) {
        NSString* result = [self.parseBuffer componentsJoinedByString:@""];
        self.parseBuffer = nil;
        return result;
    } else {
        self.parseBuffer = nil;
        return text;

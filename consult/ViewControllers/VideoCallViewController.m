...
@interface VideoCallViewController () <UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, QBRTCClientDelegate, LocalVideoViewDelegate>
...
@property (assign, nonatomic) BOOL isOpponentCollectionViewCellLoaded;
...
-(instancetype)initWithSesstionsAndOpponentUsers:(QBRTCSession*)session opponentUsers:(NSArray*)opponentUsers {
    ...
    if (self) {
        NSMutableArray* users = [NSMutableArray arrayWithArray:opponentUsers];
        [users insertObject:[LEAccountService sharedService].account.user atIndex:0];
        self.users = users;
        self.session = session;
        self.isOpponentCollectionViewCellLoaded = NO;
    }
    return self;
}
...
- (void)viewDidLoad {
    ...
    _isOffer = ([LEAccountService sharedService].account.userId == [self.session.initiatorID intValue]);
...
- (UIView *)videoViewWithOpponentID:(NSNumber *)opponentID {
    ...
    if ([LEAccountService sharedService].account.userId == [opponentID intValue]) {
        if (!result) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(!self.isOpponentCollectionViewCellLoaded) {
        UINib *nib = [UINib nibWithNibName:@"OpponentCollectionViewCell" bundle: nil];
        [collectionView registerNib:nib forCellWithReuseIdentifier:kOpponentCollectionViewCellIdentifier];
        self.isOpponentCollectionViewCellLoaded = YES;
    }
    ...
    if ([LEAccountService sharedService].account.userId == user.ID) {
        cell.connectionState = QBRTCConnectionConnected;
    }
    return cell;
}
...
- (NSIndexPath *)indexPathAtUserID:(NSNumber *)userID {
    QBUUser* user = [[self.users filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"SELF.ID == %d", [userID intValue]]] objectAtIndex:0];
